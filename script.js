    ...
    const services = [
        {
            name: 'ВКонтакте',
            iconClass: 'icon-vk',
            icon: '<svg width="28" height="28" viewBox="0 0 24 24" fill="white"><path d="M12.785 16.241s.288-.032.436-.194c.136-.148.132-.427.132-.427s-.02-1.304.587-1.496c.598-.19 1.365 1.26 2.179 1.816.615.42 1.083.328 1.083.328l2.175-.03s1.138-.07.598-.964c-.044-.073-.313-.661-1.608-1.869-1.356-1.264-1.174-1.06.459-3.247.992-1.33 1.389-2.142 1.265-2.49-.118-.331-.845-.244-.845-.244l-2.45.015s-.182-.025-.317.056c-.131.079-.216.263-.216.263s-.387 1.028-.903 1.903c-1.089 1.85-1.524 1.948-1.702 1.834-.414-.266-.31-1.07-.31-1.64 0-1.782.27-2.525-.525-2.713-.264-.063-.458-.105-1.133-.112-.866-.009-1.599.003-2.014.207-.277.136-.491.439-.361.456.161.021.526.098.72.362.25.34.241 1.107.241 1.107s.143 2.098-.334 2.358c-.327.178-.775-.185-1.736-1.846-.492-.85-.863-1.79-.863-1.79s-.072-.176-.2-.272c-.155-.115-.372-.151-.372-.151l-2.328.015s-.35.01-.478.162c-.114.135-.009.414-.009.414s1.817 4.244 3.875 6.384c1.886 1.963 4.03 1.834 4.03 1.834h.971z"/></svg>',
            url: `https://vk.com/share.php?url=${encodeURIComponent(pageUrl)}&comment=${encodeURIComponent(shareText)}&image=${encodeURIComponent(imageUrl)}`
        },
        {
            name: 'Telegram',
            iconClass: 'icon-telegram',
            icon: '<svg width="28" height="28" viewBox="0 0 24 24" fill="white"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>',
            url: `https://t.me/share/url?url=${encodeURIComponent(pageUrl)}&text=${encodeURIComponent(shareText)}`
        },
        {
            name: 'Одноклассники',
            iconClass: 'icon-ok',
            icon: '<svg width="28" height="28" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#EE8208"/><text x="12" y="16.5" text-anchor="middle" font-family="Arial, sans-serif" font-weight="bold" font-size="11" fill="white">ОК</text></svg>',
            url: `https://connect.ok.ru/ok?st.cmd=WidgetSharePreview&st.title=${encodeURIComponent(title)}&st.shareUrl=${encodeURIComponent(pageUrl)}&st.imageUrl=${encodeURIComponent(imageUrl)}`
        },
        {
            name: 'Скопировать',
            iconClass: 'icon-copy',
            icon: '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>',
            action: 'copy'
        }
    ];
    ...
    modal.innerHTML = `
        <div class="share-modal-content">
            ...
            <div class="share-modal-header">
                <div class="share-modal-header-icon">
            ...
            ${imageUrl ? `
                <div class="share-preview">
                    <img src="${imageUrl}" alt="${title}" class="share-preview-image">
                </div>
                <hr class="share-modal-divider">
            ` : ''}
            <div class="share-services">
                ${services.map(service => `
                    <button class="share-service-btn" data-url="${service.url || ''}" data-action="${service.action || 'open'}">
                        <span class="share-service-icon ${service.iconClass}">${service.icon}</span>
            ...
            <div class="share-link-section">
                <div class="share-link-label">Ссылка</div>
                <div class="share-link-row">
                    <input type="text" class="share-link-input" value="${pageUrl}" readonly>
                    <button class="share-copy-btn" data-url="${pageUrl}">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
